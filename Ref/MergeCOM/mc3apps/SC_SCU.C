 ...
 *               service class.  The application has a 
 *               number of features:
 ...
 *                 syntaxes.
 *               - The application will send a storage commitment request
 *                 to the same SCP as it is sending images.  The storage
 *                 commitment request will be for the images included on
 *                 the command line.
 *
 *************************************************************************
 *
...
static SAMP_BOOLEAN AddInstanceToList(
                        int                 A_msgID,
                        InstanceNode**      A_list);
...
int main(int argc, char** argv)
{
    ...
    {       
        ...
        for (imageCurrent = options.StartImage; imageCurrent <= options.StopImage; imageCurrent++)
        {
            ...
            /*
             * Save storage commitment information
             */
            sampBool = AddInstanceToList( messageID, 
                                          &instanceList );
            if (!sampBool)
            {
                printf("Warning, cannot add SOP instance to Storage Commitment List [%s]\n", fname);
            }
        ...
        STORAGE_OPTIONS*    A_options )
{
    ...
    &&  !A_options->ServiceList[0] 
     && ( A_options->RemotePort != -1))
    {
        strcpy(A_options->ServiceList, "Storage_Service_List");
    }
 ...
 *
 *  Parameters  :   A_msgID    - The message ID of the message to be added
 *                               to thelist.
 *                  A_list     - List of nodes.
 *
 *  Returns     :   SAMP_TRUE
 *                  SAMP_FALSE
 *
 *  Description :   Add information about the SOP Instance to the list of
 *                  images sent on this association.
 *
 ****************************************************************************/
static SAMP_BOOLEAN AddInstanceToList(
                              int               A_msgID,
                              InstanceNode**    A_list)
{
    ...
    node = (InstanceNode*)malloc(sizeof(InstanceNode));
    if (!node)
    {
 ...
 *                  sent.  When completed, the list of objects is freed.
 *
 ****************************************************************************/
static SAMP_BOOLEAN StorageCommitment(
                        STORAGE_OPTIONS*    A_options,
                        ...
                        InstanceNode**      A_list)
{
    ...
    /*
     * Free the storage commitment objects list
     */
    while (*A_list)
    {
        node = *A_list;
