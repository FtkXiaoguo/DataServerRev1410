================================================================================
MICROSOFT FOUNDATION CLASS ライブラリ: PXServerMonitor プロジェクトの概要
===============================================================================

アプリケーション ウィザードが作成した PXServerMonitor には Microsoft 
Foundation Class の基本的な使い方が示されています。アプリケーション作成のひな型
としてお使いください。

このファイルには PXServerMonitor アプリケーションを構成している各ファイ
ルの概要説明が含まれます。

PXServerMonitor.vcproj
...
PXServerMonitor.h
   ...
   CPXServerMonitorApp アプリケーション クラスを宣言します。

PXServerMonitor.cpp
   これは、アプリケーション クラス CPXServerMonitorApp を含むメイン アプリケー
   ション ソース ファイルです。
...
res\PXServerMonitor.ico
   これは、アプリケーションのアイコンとして使用されるアイコンファイルです。この
   アイコンはメイン リソース ファイル PXServerMonitor.rc に含まれていま
   す。

res\PXServerMonitor.rc2
...
PXServerMonitorDlg.h, PXServerMonitorDlg.cpp - ダイアログ
   これらのファイルは CPXServerMonitorDlg クラスを含みます。このクラスはアプ
   リケーションのメイン ダイアログの動作を定義します。ダイアログ テンプレートは
   Microsoft Visual C++ で編集可能な PXServerMonitor.rc に含まれます。
...
StdAfx.h, StdAfx.cpp
   ...
   PXServerMonitor.pch や既にコンパイルされた型のファイル StdAfx.obj を
   ビルドするために使用されます。
