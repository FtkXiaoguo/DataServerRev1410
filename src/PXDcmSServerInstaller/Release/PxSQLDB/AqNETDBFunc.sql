...
IF NOT EXISTS (select * from AqNETDB.dbo.dbinfo  
...
FROM AqNETHistDB.dbo.patientStudy pst 
JOIN AqNETHistDB.dbo.patientSeries pss ON pst.studyIndex =  pss.studyIndex
JOIN AqNETHistDB.dbo.SeriesEventLog sel ON sel.Acton = pss.seriesIndex
GO
...
ALTER DATABASE AqNETDB SET ARITHABORT ON 
...
BEGIN
	...
	IF @AccountID <> 0
		EXEC AqNETHistDB.dbo.MakeUserObject @Username, @LastName, 
...
BEGIN
	...
	IF @AETitle <> ''
		EXEC AqNETHistDB.dbo.MakeAEObject @IsLocal, @AETitle, @AEName, @HostName,
...
BEGIN
	...
	begin
		SET @Attribute = 0
		Select @Attribute=ID From AqNETHistDB.dbo.SeriesAttribute
			Where Name=@Name and Type=@Type and Subtype=@Subtype
		IF @Attribute = 0
		begin
			Insert AqNETHistDB.dbo.SeriesAttribute Values(@Name, @Type, @Subtype, 
	...
	EXEC AqNETHistDB.dbo.MakePatient  
		@StudyInstanceUID, 
		@PatientsName, 
		@PatientID, 
		@PatientsBirthDate, 
...
AS
SELECT dbo.InstanceLevel.*, 'AqNETDB' as DBName FROM dbo.InstanceLevel
-- delete AqNETDB2 and AqNETDB3 2010/03/15 By K.Ko
GO
...
USE AqNETHistDB
print 'create procedure GetDBSizeInfo in AqNETHistDB'
EXEC (@X)
GO
...
BEGIN
	...
	--'Get AqNETHistDB data size information'


	EXEC AqNETHistDB.dbo.GetDBSizeInfo
...
BEGIN
	...
	BEGIN 
		...
		BEGIN 
			...
			SELECT @tagFilterID = ID FROM AqNETDB.dbo.TagFilter WHERE AqNETDB.dbo.TagFilter.Name = @tagFilterName
			IF (@tagFilterID <> @tmpTagFilterID) continue
			ELSE break
		END
		INSERT INTO AqNETDB.dbo.TagFilter VALUES(@tagFilterName, 'AqNET studies assignment')				
		SELECT @tagFilterID=ID FROM AqNETDB.dbo.TagFilter WHERE AqNETDB.dbo.TagFilter.Name = @tagFilterName
...
print 'create procedure DefragAqNETDB'
IF EXISTS (SELECT name FROM sysobjects 
         WHERE name = 'DefragAqNETDB' AND type = 'P')
   DROP PROCEDURE DefragAqNETDB
GO

CREATE PROCEDURE DefragAqNETDB(@dbname varchar(64), @oneInstanceTopDBSize int, @CDriveSpaceMB int)
AS
BEGIN
	...
	print 'Check '+@dbname 
	EXEC AqNETDB..GetInstanceSizeInfo @dbname, @InstanceSize output, @table_size_KB output
...
BEGIN 
	...
	SET @jobID = -1; 
	SET @processorID = -1;
	SELECT @processorID = ID FROM AqNETDB.dbo.DataProcessor WHERE ProcessName = @ProcessorName
	if(@processorID < 0) return -1; -- failed

	DECLARE @c_jobs CURSOR 
	SET @c_jobs =CURSOR LOCAL SCROLL FOR
	SELECT DISTINCT JobID FROM AqNETDB.dbo.DataJobProcessList WHERE Processor = @processorID 
	FOR READ ONLY
