================================================================================
MICROSOFT FOUNDATION CLASS ライブラリ: testDeltaViewRunCmd プロジェクトの概要
===============================================================================

アプリケーション ウィザードが作成した testDeltaViewRunCmd には Microsoft 
Foundation Class の基本的な使い方が示されています。アプリケーション作成のひな型
としてお使いください。

このファイルには testDeltaViewRunCmd アプリケーションを構成している各ファイ
ルの概要説明が含まれます。

testDeltaViewRunCmd.vcproj
...
testDeltaViewRunCmd.h
   これはアプリケーションのメイン ヘッダー ファイルです。このファイルには、
   Resource.h を含む、その他のプロジェクト固有のヘッダーが含まれていて、
   CtestDeltaViewRunCmdApp アプリケーション クラスを宣言します。

testDeltaViewRunCmd.cpp
   これは、アプリケーション クラス CtestDeltaViewRunCmdApp を含むメイン アプリケー
   ション ソース ファイルです。

testDeltaViewRunCmd.rc
...
res\testDeltaViewRunCmd.ico
   これは、アプリケーションのアイコンとして使用されるアイコンファイルです。この
   アイコンはメイン リソース ファイル testDeltaViewRunCmd.rc に含まれていま
   す。

res\testDeltaViewRunCmd.rc2
...
testDeltaViewRunCmdDlg.h, testDeltaViewRunCmdDlg.cpp - ダイアログ
   これらのファイルは CtestDeltaViewRunCmdDlg クラスを含みます。このクラスはアプ
   リケーションのメイン ダイアログの動作を定義します。ダイアログ テンプレートは
   Microsoft Visual C++ で編集可能な testDeltaViewRunCmd.rc に含まれます。
...
StdAfx.h, StdAfx.cpp
   ...
   testDeltaViewRunCmd.pch や既にコンパイルされた型のファイル StdAfx.obj を
   ビルドするために使用されます。
