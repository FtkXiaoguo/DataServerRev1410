...
TestEvtLockDaemonConfigFile::TestEvtLockDaemonConfigFile()
{
}

void TestEvtLockDaemonConfigFile::Init()
{
...
//	strcpy(m_defaultLocalAETitle, kDefaultLocalAETitle);
	...
	configFile = configRootPath+"tstEventLockDaemon.cfg";
	strcpy(gConfig.m_serverNameStr, "tstEventLockDaemon");
//	strcpy(gConfig.m_logFilename,   "c:/AQNetLog/tstEventLockDaemon.log");

	std::string LogRootPath ;
	AQNetConfiguration::GetLogFilesLocation(LogRootPath);
	TRPlatform::MakeDirIfNeedTo(LogRootPath.c_str());

	strcpy(gConfig.m_logFilename,   (LogRootPath+"tstEventLockDaemon.log").c_str());
...
void TestEvtLockDaemonConfigFile::InitKVPMap()
{
	if (GetCount() == 0)
	{
...
#endif
		Add("GlobalResourceName", &m_GlobalResourceName);
		
	}
}

extern TRLogger gRoutingLoggerHCS;
void TestEvtLockDaemonConfigFile::ValidateAndFixConfiguration()
{
...
void TestEvtLockDaemonConfigFile::LogVersionInfo(void)
{
	char buf[256];
