========================================================================
       MICROSOFT FOUNDATION CLASS ライブラリ : tstEventLock
========================================================================


AppWizard が生成した tstEventLock には、Microsoft Foundation Class の基本的な
使い方が示されています。アプリケーション作成のひな型としてお使いください。

このファイルには tstEventLock を構成している各ファイルの概要説明が含まれています。
...
tstEventLock.h
    ...
    みます） をインクルードし、また CTstEventLockApp アプリケーションクラスを宣
    言します。

tstEventLock.cpp
    このファイルは CTstEventLockApp アプリケーションクラスを含むアプリケーション
    の中心となるソースファイルです。
...
res\tstEventLock.ico
    ...
    ルです。このアイコンはリソース ファイル tstEventLock.rc によってインクルード
    されます。
...
tstEventLockDlg.h, tstEventLockDlg.cpp - ダイアログ
    これらのファイルは CTstEventLockDlg クラスを含みます。このクラスは 
    アプリケーションのメイン ダイアログの動作を定義します。ダイアログ
    テンプレートは Microsoft Visual C++ で編集可能な tstEventLock.rc に含
    まれます。
...
StdAfx.h, StdAfx.cpp
    これらのファイルはプリコンパイル済ヘッダー ファイル (PCH) tstEventLock.pch 
