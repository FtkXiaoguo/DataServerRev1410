...
//
	...
	printf("  m_seriesNN: %d \n",m_seriesNN);
	{
		...
		m_Logger.LogMessage("             m_seriesNN:%d \n",m_seriesNN);
	...
	//	for(int i=0;i<this->m_sendLoopNN;i++){
		...
		//途中,送信中断された場合
			...
			if(openAssociation())
			{
				...
				printf("try to re-open association failed \n");
			//	TRPlatform::sleep(50);
				::Sleep(1000);
				success_flag = false;
				continue;
		...
		if(m_sourceDataType == 0){
			if(success_flag){
				 ...
				 if(sendNewSeriesFlag){

					sendNewSeriesFlag  = false;
					if(!sendStudyData()){
						::Sleep(10000);
						success_flag = false;
						//
						printf(">>> sendStudyData failed \n");
						continue;
					}else{
						success_flag = true;
					}
