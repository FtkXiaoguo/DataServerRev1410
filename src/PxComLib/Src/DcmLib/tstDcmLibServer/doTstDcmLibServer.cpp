...
#if 1
	pDcmXtUtil->createServiceList(false/*isPropose*/,ServiceList);

//	pDcmXtUtil->addSOPClassUID(false/*isPropose*/,
	pDcmXtUtil->setNetworkTransferSyntax(false/*isPropose*/,DcmXT_EXS_LittleEndianExplicit,ServiceList);
	 
	pDcmXtUtil->setMaxPDUSize(false/*isPropose*/,5*1024,ServiceList);
#endif
	///////
...
//	mcStatus = MC_Set_Int_Config_Value( TCPIP_LISTEN_PORT,105 );
	mcStatus = MC_Set_Int_Config_Value( TCPIP_LISTEN_PORT,6103 );
    if (mcStatus != MC_NORMAL_COMPLETION)
    {
        return false;
    }
	...
	for(int i=0;i<10000;i++){
		 ...
		 */
		mcStatus = MC_Wait_For_Association( ServiceList,
											 1,
											   &applicationID,
											   &associationID);
	  ...
	  //      printf("\t%s\n", MC_Error_Message(mcStatus));
		...
		*/
		...
		if (mcStatus != MC_NORMAL_COMPLETION)
		{
			printf("MC_Close_Association failed %d ", mcStatus);
	//        MC_Abort_Association(&associationID);
	//		return(EXIT_FAILURE);
		}
		//
