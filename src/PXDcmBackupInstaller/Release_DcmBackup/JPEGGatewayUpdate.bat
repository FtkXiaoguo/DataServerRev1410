...
%DST_FOLDER%\utils\ModifyConfigure.exe %DST_FOLDER%\config\PXDcmSServer.cfg useJPEGGateway_AE 0 >> %LOG_FILE%

echo utils\ntrights.exe +r SeServiceLogonRight -u finecube >> %LOG_FILE%
%DST_FOLDER%\utils\ntrights.exe +r SeServiceLogonRight -u prexion

echo sc config PXDcmJobProc obj= .\finecube password= finecube >> %LOG_FILE%
sc config PXDcmJobProc obj= .\finecube password= prexion
