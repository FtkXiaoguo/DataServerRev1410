...
#include "PXDcmSServerConfigFile.h"
...
TIDICOMServerConfigFile::TIDICOMServerConfigFile()
{
}

void TIDICOMServerConfigFile::Init()
{
	m_port = 105;
...
//	AppComConfiguration::GetConfigRootPath(configRootPath);
	...
	configFile = configRootPath+"PXDcmSServer.cfg";
	strcpy(gConfig.m_serverNameStr, "PXDcmSServer");
...
//	strcpy(gConfig.m_logFilename,   "c:/AQNetLog/PXDcmSServer.log");
	...
	strcpy(gConfig.m_logFilename,   (LogRootPath+"PXDcmSServer.log").c_str());

//	gConfig.m_logFilename1 = gConfig.m_logFilename + 11;
	//strcpy(gConfig.m_mergeLog,      "c:/AQNetLog/PXDcmSServerDcmAPI.log");
	strcpy(gConfig.m_mergeLog,      (LogRootPath+"PXDcmSServerDcmAPI.log").c_str() );
//	gConfig.m_mergeLog1 = gConfig.m_mergeLog + 11;
	
	strcpy(gConfig.m_configFilename, configFile.c_str() );
	strcpy(gConfig.m_mergeCap,"c:/AQNetLog/PXDcmSServerMerge.cap");
	...
	m_postDicomResponse = 1; //2010/03/16 k.ko #660
	//
	m_seriesCompleteAutoRoutingTimeout = 1;//Sec
	m_AutoRoutingBlockSize = 64;
	//#21 2012/05/29 K.Ko
	m_AutoRoutingTrig = AutoRouringTirg_AssociationClose;//AutoRouringTirg_BlockSize; 
	//
	m_useDentalCT_AE = 0;
	m_useJPEGGateway_AE = 0;//#70

	m_safetyCheck = 1;////#27 2012/06/14 K.Ko

	//#60 2013/07/03
	m_SqlTopNumber = 0; //default 0: none
	m_SqlOrderByStudyDate = 0;//default 0 none,1: ASC,2: DESC
	m_listFromSqlAlwaysUTF8 = 1;
}



#include "nmobject.h"
void TIDICOMServerConfigFile::InitKVPMap()
{
	if (GetCount() == 0)
	{
...
*/
		...
		Add("postDicomResponse", &m_postDicomResponse); //2010/03/16 k.ko #660
		Add("traceDicomLevel",&m_traceDicomLevel);// 2010/04/27 K.Ko

		Add("AutoRoutingBlockSize",&m_AutoRoutingBlockSize );
		Add("AutoRoutingTrig",&m_AutoRoutingTrig );//#21 2012/05/29 K.Ko
		//
		Add("useDentalCT_AE",&m_useDentalCT_AE );

		Add("useJPEGGateway_AE",&m_useJPEGGateway_AE);//#70

		//#27 2012/06/14 K.Ko
		Add("safetyCheck",&m_safetyCheck );

		//#60 2013/07/03
		Add("SQL_TOP",&m_SqlTopNumber); 
		Add("SQL_ORDERBY_STUDYDATE",&m_SqlOrderByStudyDate);
		//#139_Viwer(#2216)_Read_From_DB_Alwasy_UTF8
		Add("ListFromSQL_ALWAYS_UTF8", &m_listFromSqlAlwaysUTF8);
...
void TIDICOMServerConfigFile::ValidateAndFixConfiguration()
{
	...
	m_postDicomResponse		= iClamp(m_postDicomResponse, 0, 1);
	/*
